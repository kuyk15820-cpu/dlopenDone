...
int spawnRoot(NSString* path, NSArray* args, NSString** stdOut, NSString** stdErr)
{
    ...
    if(stdOut || stdErr)
    {
        logQueue = dispatch_queue_create("com.tester.LogCollector", NULL);
...
static void github_fetchLatestVersion(NSString* repo, void (^completionHandler)(NSString* latestVersion))
{
    ...
    NSURLSessionDataTask* task = [NSURLSession.sharedSession dataTaskWithURL:githubLatestAPIURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
    {
        if(!error && [response isKindOfClass:[NSHTTPURLResponse class]])
        {
            ...
            if (!jsonError && jsonResponse[@"tag_name"])
            {
